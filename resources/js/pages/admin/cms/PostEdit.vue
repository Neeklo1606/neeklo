<template>
    <div class="post-edit">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <router-link to="/admin/posts" class="text-sm text-muted-foreground hover:text-foreground mb-2 inline-block">← Посты</router-link>
                <h1 class="text-2xl font-bold text-foreground">{{ isCreate ? 'Новый пост' : 'Редактирование поста' }}</h1>
            </div>
        ...
        <template v-else>
            <div class="rounded-lg bg-card border border-border p-6 mb-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.slug" @input="slugManuallyEdited = true" type="text" placeholder="post-slug" class="w-full px-3 py-2 rounded-lg bg-background border border-border text-foreground font-mono" />
                    </div>
                    ...
                    <div>
                        ...
                        <textarea v-model="form.body" rows="8" class="w-full px-3 py-2 rounded-lg bg-background border border-border text-foreground"></textarea>
                    </div>
                </div>
            </div>
            ...
            <div v-if="!isCreate && postId" class="rounded-lg bg-card border border-border p-6 mb-6">
                <MediaManager
                    entity-type="posts"
                    :entity-id="postId"
                    :media-collections="postData?.media_collections || {}"
                    :collections-config="[
            ...
            <div class="mt-6 flex gap-3">
                ...
                <router-link to="/admin/posts" class="px-6 py-2 rounded-lg border border-border hover:bg-muted">Отмена</router-link>
            </div>
        </template>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            postData: null,
            form: { title: '', slug: '', status: 'draft', published_at: '', excerpt: '', body: '', seo_title: '', seo_description: '' },
            taxonomyIds: [],
        };
    },
    ...
    methods: {
        ...
        async fetchItem() {
            ...
            this.error = null;
            try {
                const { data } = await axios.get(`/api/admin/cms/posts/${this.postId}`);
                const s = data.data;
                this.form = {
                    title: s.title || '',
                    slug: s.slug || '',
        ...
        async save() {
            ...
            const payload = {
                ...
                slug: this.form.slug || this.slugify(this.form.title) || 'post',
            ...
            try {
                if (this.isCreate) {
                    const { data } = await axios.post('/api/admin/cms/posts', payload);
                    const newId = data.data.id;
                    await axios.post(`/api/admin/cms/posts/${newId}/taxonomies/sync`, { taxonomy_ids: this.taxonomyIds || [] });
                    this.$router.replace(`/admin/posts/${newId}`);
                } else {
                    await axios.put(`/api/admin/cms/posts/${this.postId}`, payload);
        ...
        async syncTaxonomies() {
            ...
            try {
                const { data } = await axios.post(`/api/admin/cms/posts/${this.postId}/taxonomies/sync`, { taxonomy_ids: this.taxonomyIds || [] });
                this.postData = data.data;
            } catch (err) {
                this.handleApiError(err, 'Ошибка сохранения таксономий');
            }
        },
        onMediaUpdated(data) { this.postData = data; },
        async handleDelete() {
            const result = await Swal.fire({ title: 'Удалить?', html: `Пост <strong>${this.form.title}</strong> будет удалён.`, icon: 'warning', showCancelButton: true, confirmButtonText: 'Да, удалить', cancelButtonText: 'Отмена', confirmButtonColor: '#dc2626' });
            if (!result.isConfirmed) return;
            try {
                await axios.delete(`/api/admin/cms/posts/${this.postId}`);
