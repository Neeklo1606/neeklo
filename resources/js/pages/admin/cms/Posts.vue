    ...
    <div class="posts-page">
        <div class="mb-6">
            <div class="flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-foreground">Посты</h1>
                    <p class="text-muted-foreground mt-1">Управление постами CMS</p>
                </div>
                <router-link
        ...
        <div v-else-if="items.length === 0" class="text-center py-8 text-muted-foreground rounded-lg bg-card border border-border">Нет постов. <router-link to="/admin/posts/create" class="text-primary hover:underline">Создать</router-link></div>
                    ...
                    <tbody class="divide-y divide-border">
                        <tr v-for="item in items" :key="item.id" class="hover:bg-muted/10">
                            ...
                            <td class="px-6 py-4 text-sm text-right">
                                <router-link :to="`/admin/posts/${item.id}`" class="px-3 py-1 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded mr-2">Edit</router-link>
    ...
    methods: {
        ...
        async fetchItems() {
            ...
            try {
                ...
                const { data } = await axios.get('/api/admin/cms/posts', { params });
        ...
        async deleteItem(item) {
            const result = await Swal.fire({ title: 'Удалить?', html: `Пост <strong>${item.title}</strong> будет удалён.`, icon: 'warning', showCancelButton: true, confirmButtonText: 'Да, удалить', cancelButtonText: 'Отмена', confirmButtonColor: '#dc2626' });
            if (!result.isConfirmed) return;
            try {
                await axios.delete(`/api/admin/cms/posts/${item.id}`);
