    ...
    <div class="case-studies-page">
        <div class="mb-6">
            <div class="flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-foreground">Кейс-стади</h1>
    ...
    methods: {
        ...
        async fetchItems() {
            ...
            try {
                ...
                const { data } = await axios.get('/api/admin/cms/case-studies', { params });
        ...
        async deleteItem(item) {
            const result = await Swal.fire({ title: 'Удалить?', html: `Кейс-стади <strong>${item.title}</strong> будет удалён.`, icon: 'warning', showCancelButton: true, confirmButtonText: 'Да, удалить', cancelButtonText: 'Отмена', confirmButtonColor: '#dc2626' });
            if (!result.isConfirmed) return;
            try {
                await axios.delete(`/api/admin/cms/case-studies/${item.id}`);
