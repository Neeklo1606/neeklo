    ...
    <div class="pages-page">
        <div class="mb-6">
            <div class="flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-foreground">Страницы</h1>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-border">
                        <tr v-for="page in pages" :key="page.id" class="hover:bg-muted/10">
                            <td class="px-6 py-4 text-sm text-foreground">{{ page.id }}</td>
                            <td class="px-6 py-4 text-sm font-medium text-foreground">{{ page.title }}</td>
                            <td class="px-6 py-4 text-sm text-muted-foreground font-mono">{{ page.slug }}</td>
                            <td class="px-6 py-4 text-sm">
                                <span
                                    class="px-2 py-1 text-xs rounded"
                                    :class="{
                                        'bg-yellow-500/20 text-yellow-700 dark:text-yellow-400': page.status === 'draft',
                                        'bg-green-500/20 text-green-700 dark:text-green-400': page.status === 'published',
                                        'bg-gray-500/20 text-gray-700 dark:text-gray-400': page.status === 'archived',
                                    }"
                                >
                                    {{ page.status }}
                                </span>
        ...
        <div
            ...
            class="mt-6 flex items-center justify-between rounded-lg bg-card border border-border p-4"
        >
            <div class="text-sm text-muted-foreground">
                Показано {{ paginationFrom }} — {{ paginationTo }} из {{ pagination.total }}
            </div>
    ...
    methods: {
        ...
        async fetchPages() {
            ...
            try {
                ...
                this.handleApiError(err, 'Ошибка загрузки страниц');
            } finally {
                this.loading = false;
            }
        },
        changePage(p) {
            if (p >= 1 && p <= (this.pagination?.last_page || 1)) {
                this.page = p;
                this.fetchPages();
            }
        },
        async deletePage(page) {
            const result = await Swal.fire({
                ...
                html: `Страница <strong>${page.title}</strong> будет удалена.`,
                icon: 'warning',
