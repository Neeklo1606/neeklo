...
export function HoverDetailCard({
  ...
  return (
    ...
    >
      <motion.div
        className="bg-card border border-border/50 rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300"
          ...
          <AnimatePresence>
            {isHovered && (
              <motion.div
                ...
                className="absolute inset-0 bg-background/60 backdrop-blur-sm flex flex-col items-center justify-center gap-3 p-4"
              >
                {videoUrl && (
                  <motion.video
                    src={videoUrl}
